...
public class NetworkingActivity extends AppCompatActivity {
    ...
    * 使用场景：zip 多个联网请求的时候，最多可以接受同时 10个接口，当数据都请求完成后，
    * 会在 apply方法回调出 各个json ，这里可以对各个接口的json作出处理，再去装配数据
    */
        ...
        Observable.zip(getCricketFansObservable(), getFootballFansObservable(),
                new BiFunction<List<User>, List<User>, List<User>>() {
                    ...
                    public List<User> apply(List<User> cricketFans, List<User> footballFans) {
                        Log.e("onObscribe","apply  "+cricketFans.size());
                        Log.e("onObscribe","apply  "+footballFans.size());
//                        List<User> userWhoLovesBoth =
//                                filterUserWhoLovesBoth(cricketFans, footballFans);
                        List<User> userWhoLovesBoth=new ArrayList<>();
                        userWhoLovesBoth.addAll(cricketFans);
                        userWhoLovesBoth.addAll(footballFans);
                ...
                .subscribe(new Observer<List<User>>() {
                    @Override
                    public void onSubscribe(Disposable d) {
                        Log.e("onObscribe","Disposable   "+d.isDisposed());
                    }
                    ...
                    public void onNext(List<User> users) {
                        ...
                        Log.e("onObscribe","userList size : "+users.size());
                        for (User user : users) {
                            Log.d(TAG, "user : " + user.toString());
                        }
                    }
                    ...
                    public void onError(Throwable e) {
                        Utils.logError(TAG, e);
                        Log.e("onObscribe","onError"+e.toString());
                    }

                    @Override
                    public void onComplete() {
                        Log.d(TAG, "onComplete");
                        Log.e("onObscribe","onComplete");
                    }
                });
    }
